...
function ModLua.init()
    ...
    playerInfoMenu.registerCallback("createInfoFrame_on_info_frame_mode",
        kNPCBankLoans.createInfoFrame_on_info_frame_mode)
    ...
    if ui_kNPCTaxis_data then
        if interactMenu.mode == "shipconsole" then
            ...
            local button = row[1]:setColSpan(4):createButton({
                ...
                highlightColor = entry.active and Helper.defaultButtonHighlightColor or
                    Helper.defaultUnselectableButtonHighlightColor,
...
function kNPCTaxis.display_on_set_buttontable(buttontable)
    if kNPCTaxis.isShowTeleportToDestinationButton == true then
        ...
        buttonrow[2]:setColSpan(2):createButton({ active = true, height = Helper.standardTextHeight }):setText(
            ReadText(1416318, 689124), { halign = "center" })
            ...
            for j, loan in ipairs(loansByLender) do
                if loan and type(loan) == "table" then
                    ...
                    if lender then
                        ...
                        row[5]:createButton({ active = playerMoney >= loan.current }):setText(ReadText(1416318, 689305),
                            { halign = "center" }) -- Pay loan
